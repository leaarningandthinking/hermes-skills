...
Claude Code supports plugins that add skills (slash commands) to your sessions. Plugins are distributed via GitHub repositories.

### Method 1: Download to Local Plugin Directory (Recommended)
...
cd ~/.claude/plugins && git clone https://github.com/user/plugin-name.git

# Use the plugin in a session
claude --plugin-dir ~/.claude/plugins/plugin-name
```

**Example - Installing Karpathy Guidelines:**
```bash
mkdir -p ~/.claude/plugins
cd ~/.claude/plugins && git clone https://github.com/forrestchang/andrej-karpathy-skills.git

# Launch with the plugin
claude --plugin-dir ~/.claude/plugins/andrej-karpathy-skills

# In the session, invoke the skill
/karpathy-guidelines
```

### Method 2: Load Plugin from URL (One-off)

```bash
# Load plugin directly from URL for a single session
claude --plugin-url https://github.com/user/plugin/archive/refs/heads/main.zip
```

### Method 3: Interactive Plugin Commands (Inside Claude Code)

Inside an interactive Claude Code session, you can use:
```
/plugin marketplace add <owner/repo>
/plugin install <plugin-name>@<skill-name>
```

**Note:** These commands only work inside the interactive TUI, not from the shell.
...
- **Anthropic Financial Services:** https://github.com/anthropics/financial-services
  - 19 financial industry plugins (equity research, financial analysis, investment banking, etc.)
- **Community Plugins:** Search GitHub for `claude-code-plugin` or `claude-plugin`
...
### Recommended Plugin Libraries

- **Anthropic Financial Services** — 19 plugins for equity research, financial analysis, IB, PE, wealth management. See `references/financial-services-plugins.md` for full catalog.
