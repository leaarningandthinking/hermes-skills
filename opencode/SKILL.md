...
OpenCode requires **two separate config files** for custom providers:

**1. Provider config** (`~/.config/opencode/opencode.json`):
```json
{
...
**2. Auth config** (`~/.local/share/opencode/auth.json`):
```json
{
  "CustomProvider": {
    "type": "api",
...
**Important:** Both files are required. The provider config alone is NOT sufficient. Create directories if they don't exist:
```bash
mkdir -p ~/.config/opencode ~/.local/share/opencode
```

### Example: 讯飞星辰 Coding Plan
