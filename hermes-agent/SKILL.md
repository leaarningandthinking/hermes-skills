...
**Pitfall: Correct skill installation formats.** A bare skill name like `hermes skills install web-access` will often fail. Use one of these formats instead:
- `hermes skills install skills.sh/<skill-name>` — for skills from the skills.sh registry
- `hermes skills install https://raw.githubusercontent.com/<org>/<repo>/main/SKILL.md` — direct GitHub URL
- Some skills require npm installation first: `npm i -g <package-name>`
- If install fails, search first: `hermes skills search <skill-name>` to find the correct source

For OpenClaw plugins (memory systems, providers, integrations), see `references/tencentdb-agent-memory.md` for installation patterns including exact version requirements and security scan pitfalls.
